...
Type objective_function<Type>::operator() ()
{
  ...
  nll -= sum(dnorm(b,0.0,exp(logSigmab),true));
  
  for (int iobs=0; iobs < nobs; iobs++) {
    lamda(iobs) = exp(beta + b(state(iobs)-1));   //fill vector of predictions for means
  }
    
  nll -= sum(dpois(count,lamda,true));          //calculate the objective function

  Type lamda_coef = exp(beta);
  Type sigma_b = exp(logSigmab);
  vector<Type> lamda_state = exp(beta + b);
  ADREPORT(lamda_coef);
  ADREPORT(sigma_b);
  ADREPORT(lamda_state);
